            ...
            "contain the expected content" which {
              ...
              "display the expected paragraph FEATURE FLAGGED" when {
                "'Transport Leaving the Border' is 'RoRo'" in {
                  ...
                  val expectedText = if (declarationType == STANDARD || declarationType == SUPPLEMENTARY) {
                    messages(if (code == RoRo) s"$prefix.roro.paragraph.optional" else exitAndReturnCaption)
                  } else messages(if (code == RoRo) s"$prefix.roro.paragraph" else exitAndReturnCaption)
